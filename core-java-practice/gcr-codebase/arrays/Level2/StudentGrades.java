...
public class StudentGrades {
    public static void main(String[] args) {
        ...
        // Step 2: Arrays for marks, percentages, grades
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Step 5: Assign grade
            if (percent[i] >= 80) {
                grade[i] = "A (Level 4, above standards)";
            } else if (percent[i] >= 70) {
                grade[i] = "B (Level 3, at standards)";
            } else if (percent[i] >= 60) {
                grade[i] = "C (Level 2, approaching standards)";
            } else if (percent[i] >= 50) {
                grade[i] = "D (Level 1, below standards)";
            } else if (percent[i] >= 40) {
                grade[i] = "E (Level 1-, too below standards)";
            } else {
        ...
        System.out.println("\n--- Student Report ---");
        System.out.printf("%-10s %-10s %-10s %-12s %-12s %-30s%n",
                "Physics", "Chemistry", "Maths", "Total", "Percent", "Grade");

        for (int i = 0; i < n; i++) {
            int total = marks[i][0] + marks[i][1] + marks[i][2];
            System.out.printf("%-10d %-10d %-10d %-12d %-12.2f %-30s%n",
